.PHONY: build test-unit test-race test-integration test-integration-fast test-cleanup test-coverage clean clean-cache install fmt vet lint vuln deps security check-context ci ci-full help
...
check-context:
	...
	@ls -lh docs/COMMANDS.md docs/commands/mcp.md docs/ARCHITECTURE.md 2>/dev/null | awk '{print $$5, $$9}'
	@echo ""
	...
	ONDEMAND=$$(ls -l docs/COMMANDS.md docs/commands/mcp.md docs/ARCHITECTURE.md 2>/dev/null | awk '{sum+=$$5} END {print int(sum/1024)}'); \
